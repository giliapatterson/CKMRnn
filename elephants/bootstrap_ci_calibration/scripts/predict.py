...
parser.add_argument("--sim_folder_file", type=str, required=False, help = "File with path to simulation folder on first line")
parser.add_argument("--sim_folder", type=str, required=False, help = "Path to simulation folder")
...
# Predict on each replicate in the simulation folder
torch.cuda.synchronize()
model_cpu = model.cpu()

# Read in data to predict on
if args.sim_folder is not None:
    sim_folder = args.sim_folder
elif args.sim_folder_file is not None:
    with open(args.sim_folder_file, 'r') as file:
        sim_folder = file.readline().strip()
spatial_labels = pd.read_csv(f"{sim_folder}spatial_labels.csv")
sim_dataset = KinDataset(spatial_labels, '.')

test_kin = sim_dataset
test_pred = np.empty(len(test_kin))
truth = np.empty(len(test_kin))
...
results = {'pred': test_pred, 'truth': truth, 'N_param': spatial_labels.loc[:,'N']}
