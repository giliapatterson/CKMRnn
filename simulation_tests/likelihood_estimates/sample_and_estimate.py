...
parser = argparse.ArgumentParser(description="Sample individual, estimate population size, and compare to true")
parser.add_argument("--parents_file", type=str, default="bearded_seal_parents.csv", help="CSV file with parents of each dead individual")
parser.add_argument("--bias", type=float, default=10, help="Sampling bias in the grid (one side is this many times as likely to be sampled as the other)")
parser.add_argument("--optsteps", type=int, default=50, help="Number of optimization steps")
args = parser.parse_args()

# Example data
parents_file = args.parents_file
popsizes_file = args.popsizes_file

# Read in necessary data and arguments
parents = pd.read_csv(parents_file)
popsizes = pd.read_csv(popsizes_file)
bias = args.bias
...
def nll_function(x, grad):
    ...
    nll_val = lf.nll(x, survival_priors, fecundity_priors, parent_counts, sibling_counts, parent_comparisons, sibling_comparisons)
    print(nll_val)
    return nll_val
...
opt.set_maxeval(args.optstep) # Set max iterations, print likelihood to see that it's decreasing
#print(nll_function([10, eta1, eta2, eta3, nu11, nu12, nu21, nu22]))
x = opt.optimize([10, eta1_tilde, eta2_tilde, eta3_tilde, nu11_tilde, nu12_tilde, nu21_tilde, nu22_tilde])
print(x)

estimatedR0 = x[0]
estimatedN = np.exp(x[0])
trueN = popsizes['N'].values[-1]

print("Estimated R0:", estimatedR0)
print("Estimated N:", estimatedN)
print("True N:", trueN)